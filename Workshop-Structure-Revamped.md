# DAT406 Workshop Structure — Kiro Implementation Spec
...
📂 Challenge Files:
   services/hybrid_search.py          → Challenge 1: vector_search()
...
async def vector_search(
    ...
    Steps:
    1. Generate query embedding via Amazon Bedrock (Cohere Embed v4)
    2. Build SQL with cosine distance operator (<=>)
    3. Apply optional filters (price, rating, category) as WHERE clauses
...
- Embedding generation with Amazon Bedrock (Cohere Embed v4, 1024 dimensions)
...
- Filter predicates as WHERE clauses on the same vector query
- pgvector 0.8.0 iterative scan for filtered vector search
...
- **Workshop:** TODO has function signature and docstring. Participant writes the embedding call, SQL query, and filter logic.
...
All data tools follow `verb_noun` or `get_*` pattern: `get_trending_products`, `get_inventory_health`, `get_price_statistics`, `get_product_by_category`, `restock_product`. The existing `semantic_product_search` will be renamed to `search_products` in a separate pass (not part of this workshop implementation).
...
| Module 1: Smart Search | 20 min | C1: vector_search() | Edit |
...
| Module 1: Smart Search | 15 min | C1: vector_search() | Edit |
...
- Product catalog loaded (~1000 products with Cohere Embed v4 embeddings and HNSW index)
...
- Amazon Bedrock model access: Claude Sonnet 4 (`us.anthropic.claude-sonnet-4-20250514-v1:0`), Cohere Embed v4
...
BEDROCK_EMBEDDING_MODEL=cohere.embed-english-v4
...
├── backend/
│   ├── services/
│   │   ├── hybrid_search.py           ← Challenge 1: vector_search()
...
For the **Builders Session**, challenges 3–9 start with the solution code already in place (the `setup-builder-session.sh` script in the builders lab content copies Module 2b/3 solutions into the app before participants arrive). Participants read and test the completed implementations.
