...
export function resolveAgentType(agentName: string): AgentType {
  ...
  if (name.includes("support") || name.includes("customer_support"))
    return "support";
...
For the **Builders Session**, challenges 3–9 start with the solution code already in place. Both formats use the **same** `bootstrap-environment.sh` (Stage 1: Code Editor setup). They differ in Stage 2:

- **Workshop** uses `bootstrap-labs.sh` — seeds DB, installs deps, starts services. All 9 challenges are TODO stubs.
- **Builders Session** uses `bootstrap-labs-builders.sh` — same as workshop bootstrap, plus copies `solutions/module2/` and `solutions/module3/` files into the app at the end, so challenges 3–9 are pre-completed. Participants only need to implement challenges 1–2 (TODO stubs).

The CFN code-editor template's `LabsBootstrapUrl` parameter points to the correct script per format. No separate setup script is needed.
