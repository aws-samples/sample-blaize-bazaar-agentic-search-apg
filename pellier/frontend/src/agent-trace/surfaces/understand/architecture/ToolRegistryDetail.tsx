...
const ToolRegistryDetail: React.FC = () => {
  const { data, loading, error, refetch } = useAgentTraceData<ArchitectureConcept[]>({
    key: 'architecture',
  });
  ...
  return (
    <DetailPageShell
      ...
      cheatSheet={[
        ...
        {
          numeral: 'iii.',
          text: 'Fixture labels on the Understand · Tools surface: fourteen shipped baseline; floor_check is the required-path exercise until its stub is replaced - then /api/agent-trace/build-state reflects shipped.',
        },
      ]}
      liveState={{
